...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    # We need three variable
    # pig_out_flag: judge the condition of pig out(hog rules)
    # scores: the sum of the dice number
    # lefover_dice: counting how mang dice left
    pig_out_flag, scores, leftover_dice = 0, 0, num_rolls
    while leftover_dice:
        curry_dice = dice()
        if curry_dice == 1:
            pig_out_flag = 1
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    assert num_rolls <= 10, "Cannot roll more than 10 dic"
    ...
    while score0 < goal and score1 < goal:
        if who == 0:  # Player0's Turn
            while True:  # do while loop
                ...
                score0 += take_turn(player0_num_rolls, score1, dice)
                say = say(score0, score1)  # Commentary
                # Check the extra_turn
                if score0 >= goal or not extra_turn(score0, score1):
                    break
        else:  # Player1's Turn
            while True:  # do-while loop similar to Player0
                player1_num_rolls = strategy1(score1, score0)
                score1 += take_turn(player1_num_rolls, score0, dice)
                say = say(score0, score1)  # Commentary
                if score1 >= goal or not extra_turn(score1, score0):
                    break
        who = other(who)
    # END PROBLEM 5
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        # Judge the hightest score
        diff = player_score - last_score
        if diff > running_high:
            print(str(diff) + " point(s)! The most yet for Player " + str(who))
            return announce_highest(who, player_score, diff)
        # Default return funtion
...
def make_averaged(original_function, trials_count=1000):
    ...
    def average(*args):
        total, ave_count = 0, 0
        while ave_count < trials_count:
            total += original_function(*args)
            ave_count += 1
        return total / ave_count
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # "*** YOUR CODE HERE ***"
    count = 10
    max_dice, max_score = 0, 0
    average_roll = make_averaged(roll_dice, trials_count)
    while count > 0:
        score = average_roll(count, dice)
        if max_score <= score:
            max_score = score
            max_dice = count
        count -= 1
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if free_bacon(opponent_score) >= cutoff or extra_turn(
